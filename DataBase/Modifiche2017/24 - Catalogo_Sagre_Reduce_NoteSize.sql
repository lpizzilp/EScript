INSERT INTO ScriptDB([DDLVersion], [ScriptName], [SwVersione], [DataInserimento], [Note])VALUES(24,'24 - Catalogo_Sagre_Reduce_NoteSize.sql', '2.4.114', CURRENT_TIMESTAMP, 'Portati a 200 caratteri il campo note del catalogo sagre')go 
...
IF EXISTS(SELECT * FROM dbo.Catalogo_Sagre)
	 EXEC('INSERT INTO dbo.Tmp_Catalogo_Sagre (Id_Sagra, Descrizione, Localita, Cap, Provincia, Image, Note)
		SELECT Id_Sagra, Descrizione, Localita, Cap, Provincia, Image, CONVERT(varchar(200), Note) FROM dbo.Catalogo_Sagre (HOLDLOCK TABLOCKX)')
GO
